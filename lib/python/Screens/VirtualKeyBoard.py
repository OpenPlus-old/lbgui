...
class VirtualKeyBoard(Screen):
		...
		for key in keys:
			...
			if png:
				...
				if screenwidth and screenwidth == 1920:
					res.append(MultiContentEntryPixmapAlphaBlend(pos=(x, 0), size=(width, 55), png=png))
				else:
					res.append(MultiContentEntryPixmapAlphaBlend(pos=(x, 0), size=(width, 45), png=png))
			else:
				width = key_bg_width
				if screenwidth and screenwidth == 1920:
					res.append(MultiContentEntryPixmapAlphaBlend(pos=(x, 0), size=(width, 55), png=self.key_bg))
					text.append(MultiContentEntryText(pos=(x, 0), size=(width, 55), font=1, text=key.encode("utf-8"), flags=RT_HALIGN_CENTER | RT_VALIGN_CENTER))
				else:
	...
	def markSelectedKey(self):
		...
		if screenwidth and screenwidth == 1920:
			self.list[self.selectedKey / 12].append(MultiContentEntryPixmapAlphaBlend(pos=(x, 0), size=(width, 55), png=self.key_sel))
		else:
